...
strclock=str(clock).split(' ')[1]
start = '07:15:00.000000'
finish ='16:45:00.000000'

finish_tf=strclock>finish
start_tf=strclock<start

while finish_tf or start_tf:
    time.sleep(900)
    clock=datetime.datetime.now()
    strclock=str(clock).split(' ')[1]
    finish_tf=strclock>finish
...
while True:
    ...
    clock=datetime.datetime.now()
    strclock=str(clock).split(' ')[1]
    start = '08:15:00.000000'
    finish ='16:45:00.000000'

    finish_tf=strclock>finish
    start_tf=strclock<start
    if (datetime.datetime.today().weekday()==4  and finish==True):
    ...
    with place().container():
        ...
        st.write('last update at: '+str(clock))
        st.write(c)
    c=c+1
    print(c)
