...
while True:
    ...
    fig=px.treemap(data, path=[px.Constant('market'),'group','ticker'],values = 'capitalization', color = 'change_percentage',color_continuous_scale=['Red','Red','Red','Red','Red','Red','Red','crimson','firebrick','gray','Green','limegreen','limegreen','lime','lime','lime','lime','lime','lime'],color_continuous_midpoint=0)
    ...
    with place().container():
        st.header('Greek Stock Market Map')
        st.plotly_chart(fig, use_container_width=True)
        st.write(c)
        st.write("This is a map that shows the Greek stock market's performance. \n The map is being refreshed every 10 minutes.")
    c=c+1
